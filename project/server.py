...
get_args.add_argument("id", type=int, required=False)
get_args.add_argument("type", type=str, required=False)
get_args.add_argument("creatorName", type=str, required=False)
get_args.add_argument("numOfLamps", type=int, required=False)
...
class LighterResource(Resource):
    ...
    def get(self):
        ...
        result = LighterModel.query
        if args['id'] is not None:
            result = result.filter_by(id=args['id'])
        if args['type'] is not None:
            result = result.filter_by(type=args['type'])
        if args['creatorName'] is not None:
            result = result.filter_by(creatorName=args['creatorName'])
        if args['numOfLamps'] is not None:
            result = result.filter_by(numOfLamps=args['numOfLamps'])
        lst = result.all()
        return lst
